...
def generate_terms(task_id, params, video_script):
    ...
    if not video_terms:
        video_terms = llm.generate_terms(
            video_subject=params.video_subject, video_script=video_script, amount=8
        )
    else:
    ...
    if not video_terms:
        error = "failed to generate video terms."
        sm.state.update_task(task_id, state=const.TASK_STATE_FAILED, error=error)
        logger.error(error)
        return None
...
def generate_audio(task_id, params, video_script):
    ...
    if not custom_audio_file or not os.path.exists(custom_audio_file):
        ...
        if not voice_name:
            error = "failed to generate audio: voice_name is empty."
            sm.state.update_task(task_id, state=const.TASK_STATE_FAILED, error=error)
            logger.error(error)
        ...
        if sub_maker is None:
            error = (
                f"failed to generate audio with voice '{voice_name}' "
                f"(parsed as '{parsed_voice_name}')."
            )
            sm.state.update_task(task_id, state=const.TASK_STATE_FAILED, error=error)
            logger.error(error)
            logger.error(
...
def _start_impl(task_id, params: VideoParams, stop_at: str = "video"):
    ...
    if not video_script or (
        ...
        sm.state.update_task(
            ...
            error=video_script or "failed to generate video script.",
        )
        return
    ...
    if not audio_file:
        ...
        sm.state.update_task(
            task_id,
            state=const.TASK_STATE_FAILED,
            error=task_state.get("error") or "failed to generate audio.",
        )
        return
    ...
    if not final_video_paths:
        sm.state.update_task(
            task_id,
            state=const.TASK_STATE_FAILED,
            error="failed to generate final videos.",
        )
        return
