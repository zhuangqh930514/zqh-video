...
class TestTaskService(unittest.TestCase):
    ...
    def test_generate_audio_empty_voice_sets_error(self):
        task_id = "empty-voice-task"
        params = VideoParams(video_subject="test", video_script="hello", voice_name="")

        with tempfile.TemporaryDirectory() as tmpdir:
            with mock.patch.object(tm.sm, "state", sm.MemoryState()):
                with mock.patch.object(tm.utils, "task_dir", return_value=tmpdir):
                    audio_file, audio_duration, sub_maker = tm.generate_audio(
                        task_id, params, "hello"
                    )

                task_state = tm.sm.state.get_task(task_id)

        self.assertIsNone(audio_file)
        self.assertIsNone(audio_duration)
        self.assertIsNone(sub_maker)
        self.assertEqual(task_state["state"], const.TASK_STATE_FAILED)
        self.assertIn("voice_name is empty", task_state["error"])
